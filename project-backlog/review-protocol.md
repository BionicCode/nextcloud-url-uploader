...
The default live artifacts are the sibling files `project-backlog/backlog-workflow.md`, `project-backlog/review-protocol.md`, `project-backlog/evidence-ledger-protocol.md`, `project-backlog/backlog.md`, and `project-backlog/evidence-ledger.md`. The package provisions exactly those five files. A project may add separately governed, project-owned lifecycle artifacts for explicitly enabled independent lanes, but it must identify their paths and authority boundaries in its backlog; this protocol does not prescribe a generic filename for them.
...
Before planning, editing, or reviewing, resolve and record what is relevant to the current phase:

- repository identity and authority role;
- current branch, HEAD, target branch, and worktree state;
- the intended topology and future execution-lease shape before activation, or the exact task-supplied baseline and finalized execution lease after activation;
...
## Pre-activation authorization review

While the selected pass remains `Locked`, review the draft handoff and authorize the proposed activation only after validating:

- pass and backlog identity;
- objective and task mode;
- intended target/pass-branch topology;
- bounded scope and write authority;
- required validation;
- stop conditions and reporting requirements;
- consistency with the backlog, selected ledger, review protocol, and repository instructions;
- the required shape of the future execution lease.

This review must not require an activation SHA, pass branch, exact pass-branch HEAD, or other runtime state that cannot exist before activation. Its successful outcome is recorded exactly as:

```text
Authorization state: Authorized for activation
```

This is authorization to perform the governed activation sequence. It is not approval to begin pass-specific execution.
...
### Post-activation execution-readiness and start gate

Before pass-specific work begins:

1. the selected pass exists exactly once in the backlog and ledger;
2. every required predecessor, including any cross-lane predecessor, has `Completed` ledger status, a fully finalized evidence row, and verified accepted-commit reachability;
3. the selected pass has `Pending` ledger status;
4. no other pass is `Pending` in the selected independently governed ledger/lane;
5. ineligible later or dependent passes remain `Locked`;
6. the active row contains no premature result or review evidence;
7. the finalized handoff identifies the exact activation commit and execution lease;
8. target/pass-branch topology, ancestry, expected HEAD, allowed post-baseline commits, and working-tree state match that lease;
9. current authorization covers the intended work.

Record the narrow post-activation result exactly as either:
...
This validation verifies actual runtime state against the already authorized handoff. It is not a second broad authorization review and does not reopen accepted product intent merely because runtime values were populated. Issue the explicit execution directive only when readiness is `Ready`. If any gate fails, do not issue that directive; stop before pass-specific work and follow the authorized recovery or coordination path.
...
- one independently governed evidence ledger defines one strictly ordered lifecycle lane;
- at most one pass is `Pending` within that ledger/lane;
- zero pending passes in a ledger/lane is valid, including during maintenance and ledger finalization;
- independently governed parallel lanes require separate ledger state plus backlog-defined authority boundaries, dependencies, and integration order;
- a `Pending` pass in one independent lane does not invalidate a `Pending` pass in another;
...
After integration and before post-merge ledger finalization, verify that the accepted result commit, except an explicit review-only `N/A`, and the review-gate closure commit remain reachable from the target branch. The finalization record preserves those commit identities; it does not substitute rewritten replacements.
...
Normal backlog-governed integration must preserve the accepted commit identities. Before integration, the accepted `Result SHA` is known unless the pass is explicitly review-only with `N/A`, and the review-gate closure commit has been created and its SHA is known.

Allowed strategies include a merge commit, a fast-forward, or another strategy demonstrably preserving the existing accepted commits and leaving them reachable from the target branch. Squash merge, rebase-and-merge, cherry-pick-based replacement, and any other identity-rewriting strategy are invalid in the normal lifecycle. Version 1 defines no rewrite or SHA-remapping protocol.
...
Omit the result check only for an explicit review-only `N/A`. The post-merge finalization commit records the delivering pull-request number and the unchanged review-gate closure SHA. Later-pass activation remains blocked until the row is fully finalized and the required reachability checks have succeeded.
...
- after integration, every real accepted result commit and the review-gate closure commit remain reachable from the target branch before post-merge finalization.
