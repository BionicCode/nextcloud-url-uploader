...
Detailed evidence-ledger state transitions and SHA semantics are defined by `evidence-ledger-protocol.md`. Mandatory evidence, review, authorization, and stop rules are defined by `review-protocol.md`.
...
* define repository opt-in, file provisioning, automatic agent or skill activation, or adopter instruction hooks;
* make the presence of the distributed files sufficient evidence that a repository has activated the workflow;
* make a downstream evidence archive authoritative for backlog or lifecycle state.

## 4. Authority and document responsibilities

### 4.1 Canonical backlog workspace

The canonical repository-local backlog-workflow workspace is:
...
`project-backlog/backlog-workflow.md` is the canonical entry point for a human, agent, automation, or future optional backlog skill. The package provisions exactly these five sibling files. It centrally enforces the three protocol files and seeds `backlog.md` and `evidence-ledger.md`, whose provisioned copies become project-owned roadmap and lifecycle state respectively.

Provisioning this workspace does not activate backlog behavior and does not make repository-wide agent instructions backlog-specific. A future optional skill may locate and load this entry point, but the skill is not the authority for the workspace path or protocol contract.

The default `project-backlog/evidence-ledger.md` represents one strictly ordered lane. A project that explicitly enables independent parallel lanes may add project-owned lifecycle artifacts under `project-backlog/`; its backlog must identify every additional ledger path, authority boundary, dependency, and integration order. The package does not provision those additional artifacts. It does not prescribe a generic filename convention for them.
...
| Project backlog                | Records stable project-owned roadmap and pass-contract data: identities, order, dependencies, scope, acceptance criteria, decisions, and exceptions |
| Backlog workflow protocol      | Defines the reusable end-to-end orchestration lifecycle                                                 |
| Evidence-ledger protocol       | Defines pass states, ledger columns, SHA roles, transitions, and invariants                             |
...
| Execution handoff              | Defines one selected pass before activation and becomes its exact runtime task contract after activation |
| Execution report               | Records what the executor did and verified                                                              |
| Review report                  | Records independent assessment of the result                                                            |
| Project evidence ledger        | Records mutable lifecycle state and accepted result/review evidence                                     |
...
Unless explicitly qualified as repository adoption or integration, **activation** in this protocol means the ledger transition that activates one project pass. Repository-level adoption and automatic workflow activation are separate integration concerns.

Engineering Evidence Archive, if integrated later, is downstream artifact storage. It does not own backlog data, ledger state, acceptance, or workflow transitions.
...
* the project backlog (`project-backlog/backlog.md` by default);
* the project evidence ledger (`project-backlog/evidence-ledger.md` by default);
* branch names;
* handoffs;
* execution reports;
* review reports;
...
### 5.4 One active ordered pass per ledger/lane

One independently governed evidence ledger represents one strictly ordered lifecycle lane. At most one pass may be `Pending` within that ledger.
...
Zero pending passes in a ledger is valid, including during inter-pass maintenance and ledger finalization.

Projects may define parallel lanes only through separately governed ledger state with explicitly independent authority boundaries, dependencies, and integration order. A pass active in one independent lane does not invalidate an active pass in another, but arbitrary multiple active rows in one ledger remain invalid. The project backlog owns cross-lane dependencies and integration boundaries.
...
* drafts the intended task contract for one selected locked pass;
* preserves the authorized contract through activation;
* finalizes actual repository and branch state after the activation commit and pass branch exist;
* inserts the exact execution lease before execution readiness is validated;
...
AI assistants, local development agents, and human review surfaces are execution tools. Product names are not workflow roles by themselves.

## 7. Workflow artifacts

### 7.1 Project backlog

The backlog records stable project-owned roadmap and pass-contract data. It may evolve through explicit workflow governance and records:

* program outcomes and boundaries;
* ordered passes;
* dependencies;
* project-specific decisions and exceptions;
* pass scope, required outputs, and acceptance criteria;
* links to authoritative supporting documents.

The backlog does not own mutable lifecycle status, accepted SHAs, run evidence, or reviewer state. Those values belong only in the project evidence ledger.
...
The handoff begins as the intended task contract for one selected locked pass. After activation and pass-branch creation, it becomes the runtime contract for that pass.
...
The evidence ledger records mutable lifecycle state and accepted evidence. Its detailed semantics are defined in `evidence-ledger-protocol.md`.
...
Do not duplicate ledger lifecycle status in the backlog index.
...
flowchart TD
    ...
    C --> D[Draft handoff while pass remains Locked]
    D --> E[Pre-activation authorization review]
    E -->|Corrections required| D
    E -->|Authorized for activation| F[Commit Locked to Pending]
    F --> G[Resolve activation SHA and create pass branch]
    G --> H[Finalize handoff with actual runtime lease]
    H --> I[Post-activation execution-readiness validation]
    I -->|Blocked| O[Stop and perform authorized recovery]
    I -->|Ready| J[Explicit execution directive]
    J --> K[Execute and validate]
    K --> L[Result review]
    L -->|Corrections required| K
    L -->|Invalid scope or unresolved decision| M[Stop and revise backlog or add pass]
    L -->|Accepted| N[Close pass]
    N --> P[Commit-preserving integration and ledger finalization]
    P --> Q[Inter-pass maintenance]
    Q --> B
    M --> A
```
...
* pre-activation authorization review of the intended handoff;
* post-activation execution-readiness validation of the actual lease;
...
1. confirm every required same-lane or cross-lane predecessor is accepted, fully finalized, and has verified accepted-commit reachability;
2. complete approved inter-pass maintenance;
3. verify that no pass is currently active in the selected independently governed ledger/lane;
...
9. perform pre-activation authorization review of the intended contract;
...
The draft handoff converts a stable pass specification into an intended task contract. It becomes execution-ready only after activation, pass-branch creation, runtime finalization, and post-activation readiness validation.
...
The draft handoff must define the required runtime-lease fields and intended repository and branch topology without inventing unavailable values. The finalized handoff, completed after the activation commit and pass branch exist, must identify:
...
## 16. Pre-activation handoff authorization review

While the selected pass remains `Locked`, review the intended handoff for:

1. exact backlog identity;
2. objective fidelity;
3. correct task mode;
4. intended repository and branch topology;
5. required structure and validation rules for the future runtime lease;
6. bounded scope;
7. correct authority;
...
The review must not require the literal activation SHA, pass branch, or other runtime values that cannot exist before activation. Its outcome must be one of:
...
Authorization of the intended handoff permits only the separately authorized activation transition. It is not approval to execute and is not approval of the future result.

### Authorization marker

An authorized handoff must be clearly marked:

```text
Authorization state: Authorized for activation
```

Do not repeat this broad authorization review merely because runtime values are populated later. Post-activation readiness is the narrow state-validation gate defined below.

## 17. Activation and execution readiness

Activation, baseline recording, branch creation, handoff finalization, and readiness validation must follow `evidence-ledger-protocol.md`.

### 17.1 Activation and branch creation

At a high level:

1. transition the selected pass from `Locked` to `Pending` in its ledger;
2. commit that activation on the target branch;
3. resolve the exact activation-commit SHA;
4. create the pass branch from exactly that commit;
5. leave the ledger's delayed baseline cell empty until closure.

### 17.2 Handoff finalization

After branch creation, finalize the handoff with the literal activation SHA, target and pass branches, expected branch HEAD, approved post-baseline commits, and expected working-tree state.

### 17.3 Post-activation execution-readiness validation

Before pass-specific work begins, validate:

1. the literal activation SHA resolves;
2. the pass branch was created exactly from that SHA;
3. target/pass branch topology and ancestry match the finalized handoff;
4. branch HEAD and every allowed post-baseline commit match the handoff;
5. the working tree has the expected clean or explicitly authorized state;
6. no unexpected post-baseline work exists;
7. the finalized handoff and actual repository state agree.

Record exactly one readiness outcome:
...
This gate validates actual runtime state. It does not reopen task intent, scope, or product authorization. If readiness is blocked, do not issue the execution directive or begin pass-specific work; perform only explicitly authorized recovery or coordination.

The active pass remains `Pending` until maintainer acceptance.

## 18. Explicit execution directive

The executor must receive an explicit execution instruction in addition to a finalized handoff whose readiness state is `Ready`.
...
Its intended contract was authorized before activation, and its finalized execution lease has passed readiness validation.
...
Result review is distinct from both pre-activation handoff authorization and post-activation execution-readiness validation.
...
Before normal integration, the accepted `Result SHA` must be known except for an explicit review-only `N/A`, and the review-gate closure commit must have been created with its exact SHA known.

After acceptance:

1. close the pass according to `evidence-ledger-protocol.md`;
2. record the accepted result and review evidence;
3. keep later passes locked;
4. integrate through a commit-preserving process that keeps the accepted result commit and review-gate closure commit reachable from the target branch;
5. verify those commit identities are ancestors of the integrated target, except that review-only passes have no result commit to verify;
6. finalize delayed ledger fields without substituting rewritten identities;
7. perform inter-pass maintenance.

Merge commits and fast-forwards are normal commit-preserving strategies. Squash merges, rebase-and-merge, cherry-pick replacement, and any other strategy that rewrites the accepted identities are not valid in the normal governed-pass lifecycle. Version 1 defines no rewrite or identity-remapping protocol.

After integration and before post-merge ledger finalization, verify reachability conceptually with:
...
Omit the result check only for an explicit review-only `N/A`. The finalization commit records the delivering pull-request number and the unchanged closure SHA; it does not replace either accepted identity.
...
* zero passes may be `Pending` in the selected ledger/lane;
* later passes in that ledger remain `Locked`;
...
3. stable backlog identity, order, dependencies, and pass contract;
4. evidence-ledger lifecycle state;
...
* confirm which pass is active in the selected ledger/lane;
* confirm that no other pass is pending in that ledger;
...
### Identity-rewriting integration

A squash, rebase, cherry-pick, or other rewrite replaces the accepted result or closure commit even though the ledger relies on its original SHA as durable target-history evidence.
...
This checklist describes governance readiness after an explicit integration mechanism has been selected. Copying or provisioning the files alone does not activate the workflow, grant write authority, install a skill, or establish an adopter instruction hook.
...
The distribution repository supplies governance and seed templates plus a separate central provisioning workflow. That workflow provides explicit adopter opt-in for file provisioning only; automatic agent or skill activation and adopter-root instruction integration remain deferred.
