...
The default project evidence ledger at `project-backlog/evidence-ledger.md` is the sole mutable lifecycle and accepted-evidence authority for its lane. Reusable orchestration, review, and ledger semantics are defined by the sibling files `project-backlog/backlog-workflow.md`, `project-backlog/review-protocol.md`, and `project-backlog/evidence-ledger-protocol.md`.
...
One independently governed evidence ledger defines one strictly ordered lifecycle lane. The default provisioned ledger uses this model: at most one pass may be `Pending` within that ledger, and zero pending passes is valid. If independent parallel lanes are explicitly permitted, identify each additional project-owned ledger path, its authority boundaries, cross-lane dependencies, and integration/merge ordering. Each lane requires separately governed ledger state. Do not assume or invent a generic filename convention for additional ledgers.
